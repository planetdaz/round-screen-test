...
#define TIMEZONE usCT // See below for other "Zone references", UK, usMT etc.
...
void decodeNTP(void)
{
  ...
  while (millis() < waitTime && !timeValid)
  {
    ...
    if (udp.parsePacket())
    {
      ...
      time_t local = TIMEZONE.toLocal(utc, &tz1_Code);
      uint8_t local_hh = hour(local);
      uint8_t local_mm = minute(local);
      uint8_t local_ss = second(local);
      
      Serial.print("Local time (");
      Serial.print(tz1_Code->abbrev);
      Serial.print("): ");
      Serial.print(local_hh);
      Serial.print(':');
      if (local_mm < 10) Serial.print('0');
      Serial.print(local_mm);
      Serial.print(':');
      if (local_ss < 10) Serial.print('0');
      Serial.println(local_ss);

      time_secs = local_hh * 3600 + local_mm * 60 + local_ss; // Update the clock time with local time
    }
  }
